(
    function (G) {
        ...
        var UtilityNamespace = {
			...
			/**
			 2.判断传入的win是否是window对象. 如果win对象不可访问(比如跨域iframe中的top),则认为其不是window对象.
			 */
            isWindow: function (win) {
                var result = false;
                try {
                    if (win && typeof win === "object" && win.document && "setInterval" in win) {
                        result = true;
                    }
                } catch (ex) {
                    result = false;
			...
			/**
			 * 3.判断当前窗口是否包含在Iframe中
			 * @name this.isInIframe
			 * @function
			 * @grammar this.isInIframe([win])
			 * @param {Window} win 要检测的window对象，不传递则为脚本执行时的window对象。
			 * @meta star
			 *
			...
			/**
			 * 4.判断win对象是否包含在跨域的iframe中.如果同时传递win和topWin，则只判断win和topWin之间是否跨域。
			 * @name this.isCrossDomain
			 * @function
			 * @grammar this.isCrossDomain([win][, topWin])
			 * @param {Window} win 要检测的window对象，默认为当前窗口的window。
			 * @param {Window} topWin 父窗口window对象，默认为top
			 * @meta star
			 *
			 * @return {Boolean} true表示win与topWin存在跨域
			 */
            isInCrossDomainIframe: function (win, topWin) {
                var result = false;
                win = win || window;
                ...
                var count = 0;
                while ((win != topWin || win != win.parent) && count < 10) {
                    count++;
                    //检查top和parent对象是否被篡改.
			...
			/**
			 * 6.发送pingback
			 * @name Utility.sendRequestViaImage
			 * @function
			 * @grammar Utility.sendRequestViaImage(url,win)
			 */
            sendRequestViaImage: function (url, win) {
                var img = new Image();
                var id = "cx_log_" + Math.floor(Math.random() * 2147483648).toString(36);
                win = win || window;
                win[id] = img;
			...
			/**
			 9.获取最顶层的window对象. 如果window在跨域iframe中, 或者top对象被修改, 则返回window本身
			 */
            getTopWindow: function (win) {
                win = win || window;
                if (this.isInIframe(win) && !this.isInCrossDomainIframe(win, win.top) && this.isWindow(win.top)) {
            ...
            getStyle: function (element, styleName) {
                ...
                if (styleName.indexOf("-") > -1) {
                    styleNameOther = styleName.replace(/[-_][^-_]{1}/g, function (match) {
                        return match.charAt(1).toUpperCase();
                    });
                } else {
                    styleNameOther = styleName.replace(/[A-Z]{1}/g, function (match) {
			...
			/**
			 12.获取元素相对于页面左上角的坐标(用于计算广告展示所在位置)
			 */
            getPositionCore: function (element) {
                ...
                var doc = this.getDocument(element), browser = this.browser,
                    ...
                    // 对于这些有bug的情况
                    // 使用递归查找的方式
                    BUGGY_GECKO_BOX_OBJECT = browser.isGecko > 0 && doc.getBoxObjectFor && this.getStyle(element, 'position') == 'absolute' && (element.style.top === '' || element.style.left === ''), pos = {
                        "left": 0,
                        "top": 0
                ...
                if (element.getBoundingClientRect) {// IE and Gecko 1.9+
                    ...
                    var htmlDom = doc.body,
                        ...
                        htmlBorderLeftWidth = parseInt(this.getStyle(htmlDom, 'borderLeftWidth')), htmlBorderTopWidth = parseInt(this.getStyle(htmlDom, 'borderTopWidth'));
                    ...
                    do {
                        ...
                        if (browser.isWebkit > 0 && this.getStyle(parent, 'position') == 'fixed') {
                    ...
                    // 对body offsetTop的修正
                    if (browser.opera > 0 || (browser.isWebkit > 0 && this.getStyle(element, 'position') == 'absolute')) {
			...
			/**
			 * 13.如果目标元素在Iframe中，获取目标元素相对于顶层文档(或第一个非跨域的iframe)左上角的位置
            ...
            getPosition: function (id, win) {
                ...
                if (!element)
                    return;
                var result = this.getPositionCore(element);
                //获取DOM元素在当前window上的位置
                var tempPos;
                //从当前DOM的window开始累加top和left偏移量
                var maxCount = 10;
                count = 0;
                while (win != win.parent && count < maxCount) {
                    ...
                    if (!this.isInCrossDomainIframe(win, win.parent) && win.frameElement) {
			...
			/**
			 * 14.获取元素的宽度，包含padding和border，可选是否包含margin（默认为false不包含）
			 * @name this.getOuterWidth
			 * @function
			 * @grammar this.getOuterWidth(element[,includeMargin])
			 * @param {HTMLElement|string} element 目标元素或目标元素的id
			 * @param {Boolean} includeMargin 是否包含margin，默认为false不包含。
			 * @meta star
			 *
			 * @return {number} 元素宽度值
			 */
            getOuterWidth: function (element, includeMargin) {
                ...
                if (includeMargin) {
                    var marginLeft = this.getStyle(element, "marginLeft").toString().toLowerCase().replace("px", "").replace("auto", "0");
                    var marginRight = this.getStyle(element, "marginRight").toString().toLowerCase().replace("px", "").replace("auto", "0");
                    result = result + parseInt(marginLeft || 0) + parseInt(marginRight || 0);
                }
                return result;
            },
			...
			/**
			 * 15.获取元素的高度，包含padding和border，可选是否包含margin（默认为false不包含）
            ...
            getOuterHeight: function (element, includeMargin) {
                ...
                if (includeMargin) {
                    var marginTop = this.getStyle(element, "marginTop").toString().toLowerCase().replace("px", "").replace("auto", "0");
                    var marginBottom = this.getStyle(element, "marginBottom").toString().toLowerCase().replace("px", "").replace("auto", "0");
                    result = result + parseInt(marginTop || 0) + parseInt(marginBottom || 0);
                }
                return result;
            },
			...
			/**
			 * 16.如果存在iframe嵌套，则返回DOM对象最外层的Iframe容器对象。如果不包含iframe嵌套或者在跨域的iframe中，则直接返回DOM对象本身。
			 ...
			 * @meta star
			 *
			 * @return {Boolean} 如果存在iframe嵌套，则返回DOM对象最外层的Iframe容器对象。如果不包含iframe嵌套或者在跨域的iframe中，则直接返回DOM对象本身。
			 */
			...
			/**
			 *17. 获取元素在其所属window中的透明度，不会计算iframe嵌套的透明度(可以用于反作弊)
                ...
                try {
                    while (node && node.tagName) {
                        ...
                        if (this.browser.ie) {
                            ...
                            try {
                                nodeOpacity = (win.getComputedStyle(node, null).opacity || 1) * 100;
                            } catch (e) {
                            }
			...
			/**
			 * 18.获取元素的透明度，如果元素嵌套在非跨域Iframe中，并尝试计算iframe的透明度并累加。
			...
			/**
			 * 19.对Date的扩展，将 Date 转化为指定格式的String
            ...
            dateToString: function (dateObj, formatString) {
                ...
                if (/(E+)/.test(formatString)) {
                    formatString = formatString.replace(RegExp.$1, ((RegExp.$1.length > 1) ? (RegExp.$1.length > 2 ? "\u661f\u671f" : "\u5468") : "") + week[dateObj.getDay() + ""]);
                }
                for (var k in o) {
			...
			/**
			 * 20.将对象{a:1, b:2}序列化成“a=1&b=2”形式的字符串
			 */
                    ...
                    switch (typeof tempValue) {
                        ...
                        case "object":
                            if (tempValue instanceof Date) {
                                result.push(tempKey + "=" + this.dateToString(tempValue, "yyyyMMddhhmmssS"));
                            }
                            break;
                        default:
                            break;
			 ...
			 * @param {Object|Array} id 元素的id或DOM元素
			 * @meta star
			 *
            ...
            md5: function (string) {
                ...
                var hightBit = ((parseInt("0x" + WordToHex(a), 16) + parseInt("0x" + WordToHex(b), 16)) % 4294967296).toString(16);
                var lowBit = ((parseInt("0x" + WordToHex(c), 16) + parseInt("0x" + WordToHex(d), 16)) % 4294967296).toString(16);
			 ...
			 *@param {window} window对象. 不传递则默认为当前窗口的window对象
			 *@meta star
			 *
			...
			/**
			 * 29.将url中的使用escape编码(%u[\d|\w]{4})格式的参数转换为encodeURIComponent格式
			 * eg: "a=%u4E2D%u56FD"转换后为:"a=%E4%B8%AD%E5%9B%BD"
			 *@name this.escapeToEncode
			 *@function
			 *@grammer var url = this.escapeToEncode("http://www.baidu.com/?a=%u1234");
			 *@param {string} url
			 *@meta star
			 *
			...
			/**
			 * 39.发送ajax请求，结束后调用callback
			*/
            ajaxLoad: function (url, callback, params, useCache, method, isAsync) {
                if (!url) {
                    return null;
                }
                ...
                if (!useCache) {
                    ...
                    url += "&_nocache_=" + Math.random();
                }

                var m = ["GET", "POST", "get", "post"];
                if (m.indexOf(method) < 0) {
                    method = "GET";
                }
			 ...
			 * 40.获取元素的offset
			 * @param {Object} 元素
			 * @returns {Object} 返回元素offset
			 */
            getOffset: function (element, win) {
                ...
                var scrollTop = win.pageYOffset || doc.documentElement.scrollTop || doc.body.scrollTop || 0;
                var scrollLeft = win.pageXOffset || doc.documentElement.scrollLeft || doc.body.scrollLeft || 0;
